...
(() => {
  ...
  function openDirectory(attempt = 0) {
    ...
    if (attempt < 24) window.setTimeout(() => openDirectory(attempt + 1), 150);
  }
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      @media (max-width: 520px) { .network-share-directory { width: 100%; } }
    `;
  ...
  function initialize() {
    ...
    document.addEventListener('click', (event) => {
      const opener = event.target.closest(`[data-open-modal="${MODAL_ID}"]`);
      if (opener) {
        setRequested(true);
        window.setTimeout(addShareButton, 120);
      }
      const closer = event.target.closest(`#${MODAL_ID} [data-close-modal]`);
      if (closer) setRequested(false);
    }, true);
